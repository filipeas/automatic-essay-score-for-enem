...
class Corpus:
    ...
    def build_corpus(self, corpus: str) -> None:
        """
        Build splits for training, development, and testing of the corpus
        :return:
        """
        training, development, testing = split_stratified_into_train_val_test(self.read_corpus(corpus),
    ...
    @staticmethod
    def read_splits(split=None, root_path="extended-corpus/splits") -> Tuple[DataFrame, DataFrame, DataFrame]:
        """
        ...
        if split:
            ...
            training = pd.read_csv(os.path.join(path, 'train.csv'), converters={'essay': eval, 'competence': eval})
            development = pd.read_csv(os.path.join(path, 'dev.csv'), converters={'essay': eval, 'competence': eval})
            testing = pd.read_csv(os.path.join(path, 'test.csv'), converters={'essay': eval, 'competence': eval})
